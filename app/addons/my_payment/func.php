...
function fn_my_payment_install() {
    ...
    $_data = array(
        ...
        'processor_script' => 'my_payment_processor.php',
        'processor_template' => 'addons/my_payment/views/orders/components/payments/my_payment.tpl',
//        'processor_template' => 'views/orders/components/payments/cc.tpl', //Credit Card template
        'admin_template' => 'my_payment_admin_template.tpl',
        'callback' => 'N',
        'type' => 'P',
        'addon' => 'my_payment'
    );
...
function fn_my_payment_uninstall() {
    db_query("DELETE FROM ?:payment_processors WHERE processor_script = ?s", "my_payment_processor.php");
}
...
//}
//
function fn_validate_my_payment_request( $data ) {

    if( ! isset(
        $data['click_trans_id'],
        $data['service_id'],
        $data['merchant_trans_id'], // это 'transaction_param' которое мы отправляем
        $data['amount'],
        $data['action'],
        $data['sign_time']
